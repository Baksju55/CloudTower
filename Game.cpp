...
void Game::play()
{
	...
	{ 
		...
		next_screen(play); // jezeli gracz przekroczyl dana wysokosc , zaladowac kolejny ekrean 
		window.display(); 
...
void Game::next_screen(player &play)
{
	if (play.getPosition().y < 200)
	{
		move_all(sf::Vector2f(0, 450));
		play.move(sf::Vector2f(0, 440));
		add_score();
	}
}
